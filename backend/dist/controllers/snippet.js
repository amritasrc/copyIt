...
async function handleGetUserSnippets(req, res) {
    const userId = req.user?.userId;
...
async function handleGetSingleSnippet(req, res) {
...
async function handleUpdateSnippet(req, res) {
    const snippetId = req.params.id;
    const userId = req.user?.userId;
    if (!mongoose.Types.ObjectId.isValid(snippetId)) {
        return res.status(400).json({
...
async function getSharedSnippet(req, res) {
    try {
...
export { handleCreateSnippet, handleGetUserSnippets, handleGetSingleSnippet, handleUpdateSnippet, handleDeleteSnippet, toggleFavorite, createShareLink, getSharedSnippet, };
